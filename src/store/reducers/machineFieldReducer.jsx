...
export const machineFieldReducer = (state = initialState, action) => {
  switch (action.type) {
    case "ADD_NEW_FIELD":
      ...
      let findTypeIndex = previousMachineTypes?.findIndex(
    ...
    case "ADD_NEW_TYPE":
      return {
        ...state,
        machineTypes: Array.isArray(state.machineTypes)
    ...
    case "EDIT_TYPE_NAME":
      ...
      let findTypeEditIndex = previousMachineEditTypes?.findIndex(
    ...
    case "UPDATE_FIELD_TYPE":
      ...
      if (findTypeIndexField !== -1) {
        ...
        let findFieldInd = updatedType.fields?.findIndex(
          (fieldItem) => fieldItem.id === action.payload.fieldId
        );
    ...
    case "UPDATE_SINGLE_FIELD":
      ...
      let findSingleTypeIndexField = previousSingleMachineTypesField?.findIndex(
        (item) => item.typeId === action.payload.typeId
      );

      if (findSingleTypeIndexField !== -1) {
        let updatedType = {
          ...previousSingleMachineTypesField[findSingleTypeIndexField],
        };
        let findFieldInd = updatedType.fields?.findIndex(
          (fieldItem) => fieldItem.id === action.payload.fieldId
        );
    ...
    case "REMOVE_FIELD":
      ...
      let findTypeIndexRemoveField = previousMachineTypesRemoveField?.findIndex(
        (item) => item.typeId === action.payload.typeId
      );
      if (findTypeIndexRemoveField !== -1) {
        let updatedType = {
          ...previousMachineTypesRemoveField[findTypeIndexRemoveField],
        };
        let findFieldInd = updatedType.fields?.findIndex(
          (fieldItem) => fieldItem.id === action.payload.fieldId
        );
        let filteredField = updatedType.fields?.filter(
          (fieldItem) => fieldItem.id != action.payload.fieldId
        );
        if (findFieldInd !== -1 && findFieldInd !== undefined) {
    ...
    case "REMOVE_TYPE":
      let filteredItem = state.machineTypes.filter(
        (item) => item.typeId !== action.payload
      );
      return {
        ...state,
